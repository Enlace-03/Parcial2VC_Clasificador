...
def calcular_hog(imagen, pixeles_por_celda=8, celdas_por_bloque=2, bins=9):
    # Definir los parámetros del HOG, con esta linea se calcula el HOG
...
def HOG(ruta):
    ...
    diferencia = cv2.absdiff(current_frame, background_model) #Aqui se hace la diferencia entre el fondo y la imagen
    diferencia_gris = cv2.cvtColor(diferencia, cv2.COLOR_BGR2GRAY) # Se pasa a escala de grises esa diferencia

    # Aplicar un umbral para obtener una máscara binaria
    _, foreground_mask = cv2.threshold(diferencia_gris, 50, 255, cv2.THRESH_BINARY) # Aqui se hace la mascara
    #Pasar a color nuevamente
    foreground_mask_color = cv2.cvtColor(foreground_mask, cv2.COLOR_GRAY2BGR)

    # Aplicar la máscara a la imagen original
    imagen_resultante = cv2.bitwise_and(current_frame, foreground_mask_color) # Aqui se toman los pixeles que estaban en blanco en la mascara y toma el color original

    imagen_resultante_gris = cv2.cvtColor(imagen_resultante, cv2.COLOR_BGR2GRAY)# Paso esa imagen a grises por que es lo que necesita el HOG
    vector_hog = calcular_hog(imagen_resultante_gris) # Se la envio a la funcion
